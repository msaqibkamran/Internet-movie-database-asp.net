...
$(document).ready(function () {
	...
	$(document).on("click", ".removethis", function (event) { // This is the changed line
		//alert("Clicked");
		var movieid = $(this).attr('id');
		var userid = $(this).attr('name');
		console.log(movieid);
		console.log(userid);
		($(this).closest('.movie-item-style-2')).remove();

		$.ajax //Create Ajax call
			({
				type: "POST", // Here we will use  "POST" method, as we are posting data
				url: "/watch_list/DeleteConfirmed", // Url of service/controller 
				...
				},//This is how we post data inside braces like {fieldName: value, fieldName2: value and so on}
	...
	$(document).on("click", ".removeMovieFromAdmin", function (event) { // This is the changed line
		//alert("Clicked");
		var movieid = $(this).attr('id');
		console.log(movieid);
		($(this).closest('.movie-item-style-2')).remove();
		...
		$.ajax //Create Ajax call
			({
				type: "POST", // Here we will use  "POST" method, as we are posting data
				url: "/movies/DeleteConfirmed", // Url of service/controller 
				dataType: "json", //Datatype 
	...
	$(document).on("click", ".removeActorFromAdmin", function (event) { // This is the changed line
		//alert("Clicked");
		...
		$.ajax //Create Ajax call
			({
				type: "POST", // Here we will use  "POST" method, as we are posting data
				url: "/actors/DeleteConfirmed", // Url of service/controller 
				dataType: "json", //Datatype 
	...
	// user has already given stars
	var stars = $('.forUSerStars').attr('id');
	if (stars != 0) {
		clicked = true;
		marked = stars;
		$('.ion-ios-star-outline').slice(0, marked).addClass('ion-ios-star');
...
function addToWatchList(event, movieID, userId) {
	...
	$.ajax //Create Ajax call
		({
			type: "POST", // Here we will use  "POST" method, as we are posting data
			url: "/watch_list/myCreate", // Url of service/controller 
			dataType: "json", //Datatype 
			...
			success: function (result) { //results returned from server on success
				...
				//$("#averageRating").text(result.avgRat);
				//$("#ratingCount").text(result.ratingCount + " Reviews");
...
function addComment(event,movieID, userId) {

	
	
	var comment = $('#commentStatement').val();
	...
	if (comment != "")
	{
		$.ajax //Create Ajax call
			({
				type: "POST", // Here we will use  "POST" method, as we are posting data
				url: "/comments/myCreate", // Url of service/controller 
				dataType: "json", //Datatype 
				...
				success: function (result) { //results returned from server on success
					...
					console.log(result.commentstatement);
					console.log(result.stars);

					//'<div id="date"><div id="rawr">' + monthName + '</div> | ' + parseInt(dateMonth, 10) + '</div>'
					$("#movieReviews").append(
						'<div + class="mv-user-review-item"> <div class= "user-infor" > <img src="../../Images/userava.png" alt=""> <div> <h3> ' + result.username + '</h3> <div class="no-star"> ' + result.stars + ' <i class="ion-android-star"> </i> <p> </p> </div> <p class="time"> </p> </div></div><p> ' + result.commentstatement + ' </p></div >'
					);
...
mouseout = function () {
	...
	$(this).prevAll('.ion-ios-star-outline').removeClass('ion-ios-star');
	if (clicked) {
		$('.ion-ios-star-outline').slice(0, marked).addClass('ion-ios-star');
	}
};
...
mouseover = function () {
	$('.ion-ios-star-outline').on('mouseout', mouseout);
	$('.ion-ios-star').removeClass('ion-ios-star');

	$(this).addClass('ion-ios-star');
	$(this).prevAll('.ion-ios-star-outline').addClass('ion-ios-star');
};

$('.ion-ios-star-outline').on('mouseout', mouseout);
$('.ion-ios-star-outline').on('mouseover', mouseover);


//$('.ion-ios-star-outline').click(function () {
function addRating(event, movieID, userId) {
	clicked = true;
	marked = $('.ion-ios-star').length;
	console.log(marked);

	$.ajax //Create Ajax call
		({
			type: "POST", // Here we will use  "POST" method, as we are posting data
			url: "/ratings/myCreate", // Url of service/controller 
			dataType: "json", //Datatype 
			...
			success: function (result) { //results returned from server on success
				...
				//$(".ratingCommentStars").text(marked);
				$("#averageRating").text(result.avgRat);
				$("#ratingCount").text(result.ratingCount + " Reviews");
